...
Xp_diff = Z_prime + 0.3 * np.random.normal(size=n)

f_1 = F_estimator()
f_2 = F_estimator()
f_1.fit(X, Z)
f_2.fit(Xp_diff, Z_prime)
...
im_diff = plt.imshow(wit_same, cmap='viridis', interpolation='nearest',
                     extent=[z_arguments_mcmd[0], z_arguments_mcmd[-1], x_arguments[0], x_arguments[-1]],)

plt.colorbar()
plt.savefig("/Users/simonperschel/Dropbox/Bachelorarbeit/Arbeit/6.Experimente/Images/Experiment_F_diff.pdf")
plt.show()